    ...
    pub fn deinit(self: *Node) void {
        switch (self.*) {
            .program => |x| {
                ...
                x.statements.deinit(x.allocator);
                const z = x.allocator;
        ...
        switch (expr.*) {
            ...
            .if_expression => |x| {
                ...
                if (x.alternative) |alt| {
                    ...
                    alt.statements.deinit(alt.allocator);
...
pub const Program = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) *Node {
        ...
        prg.statements = std.ArrayList(*Ast.Statement).initCapacity(allocator, 0) catch @panic("OOM");
...
test "TestString" {
    ...
    var statements = try std.ArrayList(*Ast.Statement).initCapacity(allocator, 0);
    ...
    try program.string(&buffer_writer);
    try std.testing.expectEqualStrings("let myVar = anotherVar;", buffer_writer.buffered());
}
